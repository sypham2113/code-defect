...
function layttHD() { //Lấy thông tin được điền vào từ phiếu hóa đơn
    ...
    if (CheckHD()) {
        ...
        // Gender
        var gioitinhValue = document.getElementById('nngioitinh').value;
        ...
        // Payments
        var ptttValue = document.getElementById('pttt').value;
        switch (ptttValue) {
            case '1':
        ...
        // Name
        var errors = []; // Mảng lưu trữ các lỗi

        // Name
        var tenkhValue = document.getElementById('nnhoten').value;
        if (!validateName(tenkhValue)) {
            // errors.push("Tên không hợp lệ. Vui lòng nhập lại");
        }
        
        // Phone Number
        var sdtkhValue = document.getElementById('nnsdt').value;
        if (!validatePhoneNumber(sdtkhValue)) {
            // errors.push("Số điện thoại không hợp lệ. Vui lòng nhập lại");
        }
        
        // Email
        var mailkhValue = document.getElementById('nnmail').value;
        if (!validateEmail(mailkhValue)) {
            // errors.push("Email không hợp lệ. Vui lòng nhập lại");
        }
        
        // Province-City
        var tpkhValue = document.getElementById('nncity').value;
        if (!validateCity(tpkhValue)) {
            // errors.push("Tỉnh/Thành phố không hợp lệ. Vui lòng nhập lại");
        }
        
        // Address
        var diachikhValue = document.getElementById('nndiachi').value;
        if (!validateAddress(diachikhValue)) {
            // errors.push("Địa chỉ không hợp lệ. Vui lòng nhập lại");
        }
        ...
        // Nếu không có lỗi, tiếp tục xử lý thông tin hóa đơn
        ...
        var arr = {
            ...
            mailkh: mailkhValue,
            tpkh: tpkhValue,
            diachikh: diachikhValue,
            ngaysinhkh: document.getElementById('nnngaysinh').value,
            pttt: thanhtoanpt,
            gioitinhkh: gioitinh,
...
function validateName(name) {
    var regex = /^[a-zA-Z ]*$/; // Sửa biểu thức chính quy để loại bỏ ký tự đặc biệt
    return name.length <= 255 && regex.test(name);
}
...
function validateEmail(email) {
    ...
    return atSymbolCount === 1 && email.length <= 255;
}

// Hàm kiểm tra tỉnh/thành phố
function validateCity(city) {
    var regex = /^[a-zA-Z0-9 ]+$/;
    return city.length <= 255 && regex.test(city);
}

// Hàm kiểm tra địa chỉ
function validateAddress(address) {
    var regex = /^[a-zA-Z0-9 ]+$/;
    return address.length <= 255 && regex.test(address);
}
        ...
        function createNewAccount() {
            ...
            var member = { username: document.getElementById('username').value, password: document.getElementById('password').value, usertype: "member", email: document.getElementById('email').value, phone: document.getElementById('phone').value };
            if (check(member)) {
        ...
        function createAdmin() {
            ...
            if (localStorage.getItem('user') == null) {
                var user1 = { username: 'admin', password: 'admin', usertype: 'admin', email: '###', phone: '###'};
        ...
        //     var specialChars = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/;
        //     var username = obj.username.trim(); // Loại bỏ khoảng trắng đầu và cuối chuỗi
            
        //     if (specialChars.test(username)) {
        //         alert("Tên người dùng không được sử dụng ký tự đặc biệt");
        //         document.getElementById('username').focus();
        //         return false;
        //     }
            
        //     if (username.length < 6 || username.length > 15) {
        //         alert("Tên người dùng phải từ 6 đến 15 ký tự");
        //         document.getElementById('username').focus();
        //         return false;
        //     }
        //     if (obj.password === "") {
        ...
        //     var password = obj.password.trim(); // Loại bỏ khoảng trắng đầu và cuối chuỗi
            
        //     if (password.length < 6 || password.length > 15) {
        //         alert("Mật khẩu phải từ 6 đến 15 ký tự");
        //         document.getElementById('password').focus();
        //         return false;
        //     }
        ...
        //     var email = obj.email.trim(); // Loại bỏ khoảng trắng đầu và cuối chuỗi
        ...
        //     var phone = obj.phone.trim(); // Loại bỏ khoảng trắng đầu và cuối chuỗi
            
        //     var phoneRegex = /^0[0-9]{1,10}$/;
        //     if (!phoneRegex.test(phone)) {
        //         alert("Số điện thoại phải bắt đầu bằng số 0 và tối đa 10 số");
        //         document.getElementById('phone').focus();
        //         return false;
        //     }
        ...
        function hideTK() {
            if (localStorage.getItem('buybyuser') != null) {
                ...
                var username = userArray[0].user;
                
                // Lấy chữ cái đầu tiên của username
                var firstLetter = username.charAt(0).toUpperCase(); // Đổi chữ cái đầu tiên thành chữ hoa
        
                // Hiển thị thông tin người dùng
                var tkkhElement = document.getElementById('tkkh');
                tkkhElement.innerHTML = 'Username: <a href="#" onclick="showtt()">' + 
                                       '<span style="background-color: orange; color: white; border-radius: 60%; padding: 10px;">' + 
                                       firstLetter +'</a>';
            }
        }
        function dangxuat() {
...
function checkforFilter(obj) {
            ...
            localStorage.setItem('cart', JSON.stringify(tempArr));
              ...
              var soLuongHienTai = parseInt(soLuongElement.innerText);
              soLuongElement.innerText = soLuongHienTai + 1;
        ...
        function createProducts() {
            ...
            if (localStorage.getItem('products') == null) {
                productarray = [
                    ...
                    { product: 1152, gender: 'kid', brand: 'adidas', img: "https://kingshoes.vn/data/upload/media/giay-adiads-EQT-BASK-ADV-BD7772-chinh-hang-tphcm-king-shoes-sneaker-authentic-tan-binh-1(1).jpg", name: "ADIDAS X LEGO", price: 69.99 },
